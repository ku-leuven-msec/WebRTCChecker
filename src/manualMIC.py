    ...
    for tx_id, packets in tqdm(turn_data.items(), desc="Verifying TURN packets"):
        for packet in packets:
            ...
            if packet.get("authentication_type") != "Long-Term":
                continue
            print(f"[DEBUG] Processing TURN packet with transaction ID: {tx_id}")
            usernameT = packet.get("credentials", {}).get("username")
            print(f"[DEBUG] Found TURN packet with username: {usernameT}")

            print(f"[DEBUG] Comparing with provided username: {username}")
            if usernameT == username:
                print(f"[ MATCH] Username matches: {usernameT}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python message_integrity_check.py <username> <password> <turn_results.json> [output.json]")
        sys.exit(1)

    username = "bff50b1b27d10beb66bdfu"
    password = "PVspPxer"

    turn_file = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else "message_integrity.json"
    main(username, password, turn_file, output_file)
